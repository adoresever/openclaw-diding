import { TOPIC_ROBOT, DWClient } from 'dingtalk-stream';
...
function createDingtalkClient(opts) {
  if (cachedClient && cachedConfig && cachedConfig.clientId === opts.clientId && cachedConfig.clientSecret === opts.clientSecret) {
    return cachedClient;
  }
...
function createDingtalkClientFromConfig(cfg) {
...
function buildMediaMsgParam(mediaId, mediaType) {
  switch (mediaType) {
    case "image":
...
async function handleDingtalkMessage(params) {
  ...
  try {
    ...
    const textChunkLimit = textApi?.resolveTextChunkLimit?.({
      cfg,
      channel: "dingtalk",
      defaultLimit: dingtalkCfg2.textChunkLimit ?? 4e3
    }) ?? (dingtalkCfg2.textChunkLimit ?? 4e3);
    ...
    const deliver = async (payload) => {
      ...
      const converted = textApi?.convertMarkdownTables ? textApi.convertMarkdownTables(rawText, tableMode) : rawText;
      const chunks = textApi?.chunkTextWithMode && typeof textChunkLimit === "number" && textChunkLimit > 0 ? textApi.chunkTextWithMode(converted, textChunkLimit, chunkMode) : [converted];
  ...
  currentPromise = new Promise((resolve, reject) => {
    ...
    try {
      client.registerCallbackListener(TOPIC_ROBOT, (res) => {
        ...
        try {
          const rawMessage = JSON.parse(res.data);
          if (streamMessageId) {
            rawMessage.streamMessageId = streamMessageId;
          }
          const content = (rawMessage.msgtype === "text" ? rawMessage.text?.content : void 0) ?? rawMessage.content?.recognition ?? "";
